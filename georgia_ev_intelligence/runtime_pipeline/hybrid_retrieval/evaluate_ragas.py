"""In-repo RAGAS evaluator using a local Ollama LLM as judge (default: qwen2.5:14b).
...
JUDGE MODEL CHOICE
------------------
``qwen2.5:14b`` is chosen as the default for one key reason:

  **Avoids self-evaluation bias** — answers are generated by ``gemma3:27b``;
  using any gemma model as judge risks the judge favouring its own family's
  phrasing and output style.  Qwen is a different model family and provides
  an independent assessment.

  With official ragas (explodinggradients 0.1.x), ``qwen2.5:14b`` works
  correctly for all five metrics including ``context_precision``.  The previous
  0.0 context_precision scores were caused by the vibrantlabsai ragas fork
  (which used ``instructor`` / OpenAI function-calling for structured output —
  incompatible with Ollama), not by the judge model being too verbose.
...
NOTE on context_precision:
  Official ragas 0.1.x scores context_precision reliably with ``qwen2.5:14b``.
  The previous 0.0 scores were caused by the vibrantlabsai ragas fork using
  ``instructor`` (OpenAI structured output) — incompatible with Ollama.
  With the official ragas text-based prompt, any capable Qwen/Llama judge works.
...
-----
    ...
    python -m ... --judge-model qwen2.5:14b --judge-timeout 300
...
Metrics computed per pipeline:
    ...
    All five metrics work with qwen2.5:14b (default) using official ragas 0.1.x.
    Judge choice is Qwen (not gemma) to avoid self-evaluation bias since answers
    are generated by gemma3:27b.
"""
...
# All five metrics work correctly with qwen2.5:14b using official ragas 0.1.x
# text-based prompting.  No special judge calibration required.
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Increase to 600 for qwen2.5:14b or larger judge models on slower hardware."
        ),
    )
