#!/usr/bin/env python3
"""Generate validated final-route JSON for each human-validated question.

Reads questions from the human-validated Excel workbook, runs every question
through the EXISTING route-generation pipeline -
``build_default_route_service().route_with_trace(question).final_route`` - and
stores only the validated ``FinalRoute`` contract for each question.

This is a thin *batch driver* around the existing pipeline. It deliberately does
NOT (per project boundaries - see ``route_generation/__init__.py``):

  * call the route executor or any retrieval pipeline,
  * generate final answers,
  * run SQL / vector / keyword / geo search or disruption analysis,
  * store raw (untrusted) LLM route output as the main result,
  * modify the input Excel file,
  * redesign or re-implement the router or validator.

Outputs (UTF-8 JSONL, overwritten each run):

  * outputs/final_routes.jsonl         - one record per successfully routed question
  * outputs/final_routes_failed.jsonl  - one record per question that raised an error

Run from the project root:

    python scripts/generate_final_routes_from_excel.py
"""
from __future__ import annotations

import argparse
import json
import sys
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

import pandas as pd

# --- Make the package importable when run as `python scripts/...` from root. ---
# There is no editable install, so the project root (parent of scripts/) must be
# on sys.path before importing georgia_ev_intelligence.*.
PROJECT_ROOT = Path(__file__).resolve().parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

# Existing final-route pipeline, re-exported from the package. No redesign.
from georgia_ev_intelligence.route_generation import build_default_route_service
from georgia_ev_intelligence.shared import config

# Strict preference: a column named exactly "question". A case-insensitive match
# (e.g. "Question") is accepted as the same column; anything else -> stop.
PREFERRED_QUESTION_COLUMN = "question"


def _utc_now_iso() -> str:
    """Current UTC time as an ISO-8601 string with a trailing ``Z``."""
    return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")


def _norm(name: Any) -> str:
    return str(name).strip().lower()


def _find_column(columns: list[str], aliases: tuple[str, ...]) -> str | None:
    """Return the original column whose normalized name matches any alias."""
    lookup = {_norm(c): c for c in columns}
    for alias in aliases:
        if alias in lookup:
            return lookup[alias]
    return None


def _resolve_question_column(columns: list[str]) -> str:
    """Prefer an exact ``question`` column; fall back to a case-insensitive match.

    Raises ``ValueError`` (with the available columns) if no question column
    exists, so the caller can print them and stop.
    """
    if PREFERRED_QUESTION_COLUMN in columns:
        return PREFERRED_QUESTION_COLUMN
    match = _find_column(columns, (PREFERRED_QUESTION_COLUMN,))
    if match is not None:
        return match
    raise ValueError(
        "No question column found. Expected a column named 'question' "
        f"(case-insensitive). Available columns: {columns}"
    )


def _cell_or_none(row: pd.Series, column: str | None) -> Any:
    """Return a trimmed string cell value, or ``None`` for missing/blank cells."""
    if column is None:
        return None
    value = row.get(column)
    if value is None:
        return None
    try:
        if pd.isna(value):
            return None
    except (TypeError, ValueError):
        pass  # non-scalar (e.g. list) - keep it
    text = str(value).strip()
    return text or None


def generate(
    input_path: Path,
    success_path: Path,
    failed_path: Path,
    sheet_name: Any = 0,
    limit: int | None = None,
) -> tuple[int, int, int]:
    """Route every question in ``input_path`` and write the JSONL outputs.

    Returns ``(n_ok, n_failed, n_skipped)``.
    """
    if not input_path.exists():
        raise FileNotFoundError(f"Input Excel not found: {input_path}")

    # dtype=object keeps cells verbatim and avoids pandas coercing things.
    # The Excel file is only read, never written.
    df = pd.read_excel(input_path, sheet_name=sheet_name, dtype=object)
    columns = [str(c) for c in df.columns]

    question_col = _resolve_question_column(columns)
    note = "" if question_col == PREFERRED_QUESTION_COLUMN else " (case-insensitive match)"
    print(f"Using question column: {question_col!r}{note}")

    # Build the existing pipeline ONCE and reuse it for every question.
    service = build_default_route_service()

    success_path.parent.mkdir(parents=True, exist_ok=True)
    failed_path.parent.mkdir(parents=True, exist_ok=True)

    n_ok = n_failed = n_skipped = 0

    with success_path.open("w", encoding="utf-8") as ok_fh, \
            failed_path.open("w", encoding="utf-8") as fail_fh:
        # Positional id/row number so each Excel row keeps a STABLE id even when
        # earlier rows are skipped: header is Excel row 1, first data row is row 2.
        for position, (_, row) in enumerate(df.iterrows(), start=1):
            if limit is not None and position > limit:
                break

            question_id = f"q{position:03d}"
            source_excel_row_number = position + 1

            question = _cell_or_none(row, question_col)
            if question is None:
                n_skipped += 1
                continue

            generated_at = _utc_now_iso()
            try:
                # The validator's trusted handoff contract - NOT the raw LLM route.
                final_route = service.route_with_trace(question).final_route

                # FinalRoute is a Pydantic model -> JSON-safe dict (enums -> str).
                # If a future pipeline returns a plain dict, serialize it directly.
                if hasattr(final_route, "model_dump"):
                    final_route_payload = final_route.model_dump(mode="json")
                else:
                    final_route_payload = final_route

                # Success records hold only the question and its computed route.
                record = {
                    "question": question,
                    "final_route": final_route_payload,
                }
                ok_fh.write(json.dumps(record, ensure_ascii=False) + "\n")
                n_ok += 1
                print(
                    f"[ok]     {question_id} (row {source_excel_row_number}) "
                    f"-> {final_route_payload.get('route')} "
                    f"[{final_route_payload.get('validation_status')}]"
                )
            except Exception as exc:  # per-row isolation: never abort the batch
                record = {
                    "question_id": question_id,
                    "source_excel_row_number": source_excel_row_number,
                    "question": question,
                    "error": f"{type(exc).__name__}: {exc}",
                    "generated_at": generated_at,
                }
                fail_fh.write(json.dumps(record, ensure_ascii=False) + "\n")
                n_failed += 1
                print(f"[FAILED] {question_id} (row {source_excel_row_number}): {record['error']}")

    return n_ok, n_failed, n_skipped


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate validated final-route JSON for each human-validated Excel "
            "question (router + validator only; no execution / no answers)."
        ),
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=Path(config.HUMAN_QA_EXCEL),
        help="Human-validated questions Excel file (default: %(default)s).",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "outputs",
        help="Directory for the JSONL outputs (default: %(default)s).",
    )
    parser.add_argument(
        "--sheet",
        default=0,
        help="Sheet name or index to read (default: first sheet).",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Optional: process only the first N rows (smoke testing).",
    )
    return parser.parse_args()


def main() -> int:
    args = _parse_args()
    success_path = args.output_dir / "final_routes.jsonl"
    failed_path = args.output_dir / "final_routes_failed.jsonl"

    print(f"Input:   {args.input}")
    print(f"Success: {success_path}")
    print(f"Failed:  {failed_path}")
    print("-" * 48)

    try:
        n_ok, n_failed, n_skipped = generate(
            input_path=args.input,
            success_path=success_path,
            failed_path=failed_path,
            sheet_name=args.sheet,
            limit=args.limit,
        )
    except ValueError as exc:
        # Missing question column: the message already lists the columns.
        print(f"ERROR: {exc}")
        return 2
    except FileNotFoundError as exc:
        print(f"ERROR: {exc}")
        return 2

    print("-" * 48)
    print(f"Routed OK : {n_ok}")
    print(f"Failed    : {n_failed}")
    print(f"Skipped   : {n_skipped} (empty question rows)")
    print(f"Wrote {success_path}")
    print(f"Wrote {failed_path}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
